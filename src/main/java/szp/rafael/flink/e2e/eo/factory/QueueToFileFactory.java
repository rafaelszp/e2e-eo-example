...
public class QueueToFileFactory {
    ...
    public static StreamExecutionEnvironment createExecutionEnvironment(ParameterTool params) {
        ...
        env.getCheckpointConfig().setMinPauseBetweenCheckpoints(1*100); // Pausa mínima entre checkpoints
        env.getCheckpointConfig().setCheckpointTimeout(100); // Timeout de checkpoint
...
//        config.set(CheckpointingOptions.FS_WRITE_BUFFER_SIZE, 64);
        config.set(StateBackendOptions.STATE_BACKEND, "rocksdb");


        if(!params.has("production")){
            config.setString("taskmanager.memory.network.max", "1gb");
            config.setString("taskmanager.memory.managed.size", "1gb");
        }
